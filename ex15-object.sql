...
-- 뷰 수정
/*CREATE OR REPLACE : 뷰가 없으면 생성 있으면 수정*/
...
-- ROWNUM : 쿼리 결과 각 행에 순차적인 번호를 할당해준다.
SELECT ROWNUM, employee_id, last_name
FROM employees
WHERE department_id = 80;

-- WITH READ ONLY : DML 작업거부
...
-- 뷰의 WITH CHECK OPTION의 조건에 위배되지 않아 변경 가능합니다.
UPDATE high_salary_employee_vu SET
salary = 26000
WHERE department_id = 100;
...
-- EMPLOYEES 테이블에 LAST_NAME 열에 대한 인덱스 생성
