...
SELECT UPPER('higgins') FROM dual; -- ~~ FROM dual; ~~의 결과값 반환
...
SELECT TRIM(' ' FROM ' Height ') FROM dual; -- 앞뒤 공백 제거
SELECT REPLACE(' Hei ght ',' ', '') FROM dual; -- 사이 공백 제거
...
-- TRUNC() - 절삭, 지정된 소수점 자릿수로 값을 전달합니다.
...
/*
날짜 조작 함수
    MONTHS_BETWEEN(date1, date2) : 두 날짜 간의 월수를 계산합니다.
...
TO_CHAR() - 날짜 또는 숫자를 문자열로 변환
    ...
    HH, HH12, HH24 - 1일 동안 시간 또는 반일 시간(1-12) 또는 전일 시간(0-23)을 반환합니다.
    MI - 분(0~59)을 반환합니다.
    SS - 초(0~59)를 반환합니다.
    FF - 밀리세컨즈(0~999)를 반환합니다.
    AM 또는 PM - 오전/오후를 나타내는 자오선표시 반환합니다.
    A.M. 또는 P.M. - 오전/오후를 나타내는 마침표가 있는 자오선 표시를 반환합니다.
*/
SELECT last_name, TO_CHAR(hire_date ,'YY-MM-DD HH24:MI:SS') AS HIREDATE
FROM employees;

-- TIMESTAMP - 날짜 정보 + 밀리세컨즈
...
-- NVL() 함수 - null 값을 실제 지정한 값으로 반환합니다. (null 연산이 안될때 사용)
